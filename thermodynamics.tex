...
\end{itemize}
\subsubsection{一般に}
ある平衡状態から別の平衡状態への変化において $Q-W \braunit{\J}$ という値はその変化の経路によらず一定値となる。
ただし、一般に $W \neq Q$ であることに注意。
\cite[pp.36]{thermo}
\subsubsection{サイクル} \label{section:熱力学第1法則_サイクル}
サイクル (同じ平衡状態へ戻ってくる変化) において  $W = Q\braunit{\J}$ となる。
\cite[pp.34]{thermo}
\subsubsection{自由膨張}
自由膨張において $Q = W = 0\braunit{\J}$ となる。\\
...
準静的等温過程において $Q = W = nRT\ln\frac{V_2}{V_1}\braunit{\J}$ (\fullref{section:熱と仕事_等温過程の仕事}) となる。\\
...
となる。
\cite[pp.48]{thermo}
\subsubsection{等温過程} \label{section:熱と仕事_等温過程の仕事}
等温過程 ($T$ が一定) かつ理想気体 ($p = \frac{nRT}{V}$) において
\begin{align*}
...
\end{align*}
となる。この断熱過程が定積過程でなくても $c_V$ を使って良い。\\
...
\begin{itemize}
  ...
  \item 断熱過程では $Q = 0$
  \item 定積過程では $Q = nc_V(T_1 - T_2)$ (\fullref{section:熱と仕事_準静的過程の吸熱} ※ $T_2 \rightarrow T_1$ という変化であることに注意)、 $W = 0$
  \item 等温過程では $Q = W (= nRT\ln\frac{V_1}{V_2})$ (\fullref{section:熱力学第1法則_準静的等温過程} ※ $V_2 \rightarrow V_1$ という変化であることに注意)
\end{itemize}
であるから、 $\left[Q - W\right]_{断熱過程} + \left[Q - W\right]_{定積過程} + \left[Q - W\right]_{等温過程} = - W_{断熱過程} + nc_V(T_1 - T_2)  = 0$ となる。
\cite[pp.61]{thermo}
...
\begin{align*}
  ...
  pV^\gamma &= \text{const.}
\end{align*}
となる。ただし $\beta = \frac{R}{c_V}, \gamma = \frac{c_p}{c_V}$ である。
\cite[pp.58]{thermo}
